...
impl ConfigFieldView {
    pub fn from_config_field(f: &ConfigField) -> Self {
        let kind = match &f.kind {
            ...
            ConfigFieldKind::Number { min, max } => ConfigKindView::Number {
                min: *min,
                max: *max,
            },
            ConfigFieldKind::Bool => ConfigKindView::Bool(f.value.as_bool().unwrap_or(false)),
            ...
            ConfigFieldKind::Url => ConfigKindView::Url,
            ConfigFieldKind::LanguageCode => ConfigKindView::LanguageCode,
            ConfigFieldKind::SemVer => ConfigKindView::SemVer,
            ConfigFieldKind::Tag { library } => ConfigKindView::Tag {
...
pub enum ConfigKindView {
    ...
    LanguageCode,
    SemVer,
    /// Tag picker; library name as debug string for display.
    Tag {
        library: String,
    },
}
...
impl PackageViewModel {
    ...
    pub fn from_manageable(pkg: &dyn fs_pkg::manageable::Manageable) -> Self {
        ...
        let instances_view: Vec<InstanceView> =
            instances.iter().map(InstanceView::from_instance).collect();
        ...
        Self {
            ...
            config_fields: config
                .iter()
                .map(ConfigFieldView::from_config_field)
                .collect(),
            build_fields: build
                .iter()
                .map(ConfigFieldView::from_config_field)
